...
import uv.tc.packetworldmovil.databinding.ActivityActualizarEstadoBinding
import uv.tc.packetworldmovil.pojo.Envio
import uv.tc.packetworldmovil.pojo.Historial
...
class ActualizarEstado : AppCompatActivity() {
    ...
    private fun enviarHistorial(jsonHistorial: String) {
        ...
        Ion.with(this@ActualizarEstado)
            .load("POST", "${Constantes().URL_WS}enviosEspeciales/app/cambios") // URL del servicio
    ...
    private fun verificarResultadoEdicion(resultado: String) {
        ...
        val msj = Gson().fromJson(resultado, Respuesta::class.java) // Convertir la respuesta a un objeto Mensaje
